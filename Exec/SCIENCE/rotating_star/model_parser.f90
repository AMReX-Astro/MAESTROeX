...
contains
  ...
  subroutine read_model_file(model_file)
    ...
    logical :: found_model, found_dens, found_temp, found_pres, found_conv_vel
    ...
    do i = 1, nvars_model_file
       ...
       if (varnames_stored(i) == "conv_vel") then
           found_conv_vel = .true.
       endif
    enddo

    ! check to see if this one contains conv_vel. If not, adjust the number of
    ! model variables
    if (.not. found_conv_vel) then
        ispec_model = ispec_model - 1
        iconvel_model = -1
        nvars_model = nvars_model - 1
    endif
...
889 format(a60)
    ...
    do i = 1, npts_model
       ...
       do j = 1,nvars_model_file
         ...
         else if (varnames_stored(j) == "conv_vel") then
             model_state(i,iconvel_model) = vars_stored(j)
             found_model = .true.

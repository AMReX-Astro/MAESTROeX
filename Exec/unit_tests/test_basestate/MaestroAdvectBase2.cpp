...
void Maestro::UpdateSpecies(const BaseState<Real>& rho0,
    ...
    BaseState<Real> rhoX0_edge(base_geom.max_radial_level+1, base_geom.nr_fine+1);
	...
	for (int n = 0; n <= base_geom.max_radial_level; ++n) {
		...
		X0_arr(n, r) = amrex::max(rhoX0_old_arr(n, r) / rho0_arr(n, r), 0.0);
	    });
	    Gpu::synchronize();
        }
	...
	for (int n = 0; n <= base_geom.max_radial_level; ++n) {
	    ...
	    const auto nr = base_geom.nr(n);
            ...
            ParallelFor(nr, [=] AMREX_GPU_DEVICE(int r) {
		if (!spherical) {
		    rhoX0_new_arr(n, r, comp) = rhoX0_old_arr(n, r, comp) -
			dt_loc / dr *
	...
	for (int n = 0; n <= base_geom.max_radial_level; ++n) {
		...
		if (rhoX0_new_arr(n, r, comp) < 0.0) {
			...
			if (comp2 != comp && rhoX0_new_arr(n, r, comp2) >= 0.0) {

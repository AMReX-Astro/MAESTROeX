...
Maestro::Evolve ()
{
	...
	Vector<Real> rho0_predicted_edge( (base_geom.max_radial_level+1)*(base_geom.nr_fine+1) );
	...
	for (auto i = 0; i < AMREX_SPACEDIM; i++) {
		for (auto j = -1; j <= 1; j+=2) {
			...
			Print() << "original density = " << dens_orig[0].norm2() << std::endl;
			...
			for (istep = start_step; istep <= max_step && t_old < stop_time; ++istep)
			{
				...
				DensityAdvance(1,sold,snew,sedge,sflux,scal_force,etarhoflux,
				               umac,w0mac,rho0_predicted_edge);

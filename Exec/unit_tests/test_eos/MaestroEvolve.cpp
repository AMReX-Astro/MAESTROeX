...
Maestro::Evolve ()
{
	...
	for (int lev=0; lev<=finest_level; ++lev) {
		...
		const auto dlogrho = (std::log10(dens_max) - std::log10(dens_min)) / (dom_hi[0] - dom_lo[0]);
		const auto dlogT = (std::log10(temp_max) - std::log10(temp_min)) / (dom_hi[1] - dom_lo[1]);
		...
		for (MFIter mfi(sold[lev], true); mfi.isValid(); ++mfi)
		{
			...
			ParallelFor(tileBox, [=] AMREX_GPU_DEVICE (int i, int j, int k) {
				...
				for (auto comp = 0; comp < NumSpec; ++comp) {
					xn_zone[comp] = metalicity / (NumSpec - 2); // all but H, He
				}
				...
				error_arr(i,j,k,1) += (eos_state.rho - dens_zone) / dens_zone;
				...
				if (eos_state.s > 0.0) {
					...
					error_arr(i,j,k,4) += (eos_state.T - temp_zone) / temp_zone;
				}
			});

        ...
        ParallelFor(tileBox, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            ...
            for (auto kk = 1; kk <= 3; ++kk) {
                for (auto jj = 1; jj <= 3; ++jj) {
                    for (auto ii = 1; ii <= 3; ++ii) {
                        ...
                        Real cx =
                            std::cos(2.0 * M_PI * Real(ii) * (x + center_p[0]) /
                                         velpert_scale_loc +
                                     phix[n]);
                        Real cy =
                            std::cos(2.0 * M_PI * Real(jj) * (y + center_p[1]) /
                                         velpert_scale_loc +
                                     phiy[n]);
                        Real cz =
                            std::cos(2.0 * M_PI * Real(kk) * (z + center_p[2]) /
                                         velpert_scale_loc +
                                     phiz[n]);

                        Real sx =
                            std::sin(2.0 * M_PI * Real(ii) * (x + center_p[0]) /
                                         velpert_scale_loc +
                                     phix[n]);
                        Real sy =
                            std::sin(2.0 * M_PI * Real(jj) * (y + center_p[1]) /
                                         velpert_scale_loc +
                                     phiy[n]);
                        Real sz =
                            std::sin(2.0 * M_PI * Real(kk) * (z + center_p[2]) /
                                         velpert_scale_loc +
                                     phiz[n]);
            ...
            for (auto n = 0; n < 3; ++n) {
                vpert[n] *= velpert_amplitude_loc * 0.5 *
                            (1.0 + std::tanh((velpert_radius_loc - rloc) /
                                             velpert_steep_loc));

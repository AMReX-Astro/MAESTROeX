...
Maestro::SpongeInit(const RealVector& rho0)
{
    ...
    // Top sponge
    Real r_top = prob_lo_r + r_edge_loc_b.array()(nr_fine);
    r_sp_outer = r_top;
	...
	if (rho0[max_lev*r] < sponge_start_density) {
	    r_sp_outer = prob_lo_r + (Real(r) + 0.5) * dr.array()(0);
	    break;
	}
    }
	...
	if (rho0[max_lev*r] < anelastic_cutoff_density) {
	    r_tp_outer = prob_lo_r + (Real(r) + 0.5) * dr.array()(0);
	    break;
	}
    }
	...
	if (rho0[max_lev*r] < 5.0e7) {
	    r_tp = prob_lo_r + (Real(r) + 0.5) * dr.array()(0);
	    break;
	}
    }

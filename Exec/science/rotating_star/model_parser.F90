...
contains
  ...
  subroutine read_model_file(model_file) 
    ...
    logical :: found_model, found_dens, found_temp, found_pres, found_enuc
    logical :: found_spec(nspec), found_omegadot(nspec)
    integer :: ipos
    character (len=256) :: header_line
    character (len=256) :: omegadot_name
...
889 format(a60)
    ...
    do i = 1, npts_model
       ...
       do j = 1,nvars_model_file
          ...
          if (varnames_stored(j) == "density") then
          ...
          else if (varnames_stored(j) == "enuc") then
             ...
             do comp = 1, nspec
                omegadot_name = "omegadot_" // spec_names(comp)
                if (varnames_stored(j) == spec_names(comp)) then
                   model_state(i,ispec_model-1+comp) = vars_stored(j)
                   found_model = .true.
                   found_spec(comp) = .true.
                   exit
                else if (varnames_stored(j) == omegadot_name) then 
                    model_state(i,ispec_model+nspec-1+comp) = vars_stored(j)
                    found_model = .true.
       ...
       if (i == 1) then
          ...
          do comp = 1, nspec
             ...
             if (.not. found_omegadot(comp)) then
                if ( parallel_IOProcessor() ) then
                   print *, 'WARNING: omegadot_', trim(spec_names(comp)), &
                        ' not provided in inputs file'
                end if
             endif
          enddo

...
void Maestro::InitLevelData(const int lev, const Real time, const MFIter& mfi,
    ...
    if (test_problem == 0 || test_problem == 1) {
	const auto test_problem_const = test_problem;
	const auto s0_arr = s0_init.const_array();

	ParallelFor(tileBox, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
	    int r = AMREX_SPACEDIM == 2 ? j : k;
	    if (test_problem_const == 1 && AMREX_SPACEDIM == 2) {
	...
	// number of cells in domain in x-direction
	const int ncell_x = geom[lev].Domain().bigEnd(0) + 1;
	...
	ParallelFor(tileBox, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
	    ...
	    Real y_offset = ncell_x/4 * sin(x * 2*M_PI / rwid);

//
// Get the root-mean-square (RMS) of a variable along the lateral direction
// x_rms = sqrt(<(x - <x>)^2>), where <x> denotes a horizontal averageG
...
int main(int argc, char* argv[])
{
...

    {
        ...
        // var_bin is now the horizontal average


        // Second pass to find the rms
                            ...
                            for (int j = lo.y; j <= hi.y; ++j) {
                                for (int i = lo.x; i <= hi.x; ++i) {
                                    if (m(i,j,k) == 0) { // not covered by fine
                                        ...
                                        var_bin_rms[index] += std::pow(fab(i,j,k,var_comp) - var_bin[index], 2.0) * vol;
        ...
        ParallelDescriptor::ReduceRealSum(var_bin_rms.data(), static_cast<int>(var_bin_rms.size()));

        //normalize and take the square root

        for (int i = 0; i < nbins; i++) {
            if (volcount[i] != 0.0) {
                var_bin_rms[i] = std::sqrt(var_bin_rms[i]/volcount[i]);
            }
        }

        // var_bin_rms is now the rms


        // now open the slicefile and write out the data
        amrex::Print() << "outputting rms of " << varname << " to " << slcfile << std::endl;
        ...
        for (auto i = 0; i < nbins; i++) {
            ...
            slicefile << std::setw(w) << h[i] << std::setw(w) << var_bin_rms[i] << std::endl;
        }
...
{
    amrex::Print() << "\n"
                   << " Find the rms fluctuation of a variable\n"
                   << " Usage: \n"
                   << "     faverage {[-v|--variable] name} plotfile\n"
                   << "\n"

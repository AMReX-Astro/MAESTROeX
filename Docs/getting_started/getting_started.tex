...
\item {\em Setup the problem's {\tt GNUmakefile}}.
  ...
  \item {\tt MPI := }
    ...
    option empty, disabling MPI.  This will build \maestroex\ in serial
  ...
  \item {\tt SDC := }
    ...
    (Spectral Deferred Corrections) source for \maestroex\ (see Chapter
  ...
  \item {\tt COMP := gfortran}
    ...
    This option specifies the Fortran compiler.  We will use {\tt
      gfortran}, which is the preferred compiler for \maestroex.
...
from \maestroex).  The {\tt GNUmakefile} there needs to be edited to

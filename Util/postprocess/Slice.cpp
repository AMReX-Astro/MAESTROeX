		  ...
		  const int deltat, const int nfiles)
{
	...
	meridion_vel_old[0].define(meridion_vel[0].boxArray(), meridion_vel[0].DistributionMap(),
				   ncomp, 0);
	...
	for (int n = 0; n < nfiles; ++n) {
		...
		for (int lev = 0; lev <= finest_level; ++lev) {
		    MultiFab::Copy(u_new[lev], pltfile_new.get(lev,x), 0, i, 1, 0);
		    MultiFab::Copy(w0_new[lev], pltfile_new.get(lev,w), 0, i, 1, 0);
		}
	    }
	    ...
	    Real dt = t_new - t_old;
	    MultiFab::Add(meridion_vel_old[0], meridion_vel_new[0], 0, 0, ncomp, 0);
	    MultiFab::Saxpy(meridion_vel[0], 0.5*dt, meridion_vel_old[0], 0, 0, ncomp, 0);

	    // set old states to new states values
	    for (int lev = 0; lev <= finest_level; ++lev) {
		MultiFab::Copy(u_old[lev], u_new[lev], 0, 0, AMREX_SPACEDIM, 0);
		MultiFab::Copy(w0_old[lev], w0_new[lev], 0, 0, AMREX_SPACEDIM, 0);
	    }
	    MultiFab::Copy(meridion_vel_old[0], meridion_vel_new[0], 0, 0, ncomp, 0);
	    t_old = t_new;
	}
...
#pragma omp parallel
#endif
        for ( MFIter mfi(meridion_vel[lev], TilingIfNotGPU()); mfi.isValid(); ++mfi ) {

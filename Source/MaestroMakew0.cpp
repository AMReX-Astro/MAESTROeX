                ...
                RealVector& delta_chi_w0, const bool is_predictor) 
{
    ...
    if (!spherical) {
        ...
        if (use_exact_base_state) {
            ...
            Makew0Sphr(w0_old, w0_force, Sbar_in, p0_minus_peosbar, 
                       delta_chi_w0, is_predictor);
        }
    }
                      ...
                      RealVector& delta_chi_w0, const bool is_predictor) 
{
        ...
        for (auto j = 0; j < numdisjointchunks[n]; ++j) {
            ...
            for (auto r = r_start_coord[n + max_lev*j]+1; 
                ...
                Real gamma1bar_p0_avg = (gamma1bar_old[n+max_lev*(r-1)]
                    + gamma1bar_new[n+max_lev*(r-1)]) *
                    (p0_old[n+max_lev*(r-1)] + 
                    p0_new[n+max_lev*(r-1)])/4.0;

                if (r < base_cutoff_density_coord[n]) {
                    if (is_predictor == 1) {
                        delta_chi_w0[n+max_lev*(r-1)] = dpdt_factor * 
                            p0_minus_peosbar[n+max_lev*(r-1)] / 
                            (gamma1bar_old[n+max_lev*(r-1)]*
                            p0_old[n+max_lev*(r-1)]*dt);
                    } else {
                        delta_chi_w0[n+max_lev*(r-1)] += dpdt_factor *
                            p0_minus_peosbar[n+max_lev*(r-1)] / 
                            (gamma1bar_new[n+max_lev*(r-1)]*
                            p0_new[n+max_lev*(r-1)]*dt);
                    }
                } else {
    ...
    // 0.0 w0 where there is no corresponding full state array
...
void 
Maestro::Makew0Sphr(const RealVector& w0_old, RealVector& w0_force, 
                    const RealVector& Sbar_in, 
                    const RealVector& p0_minus_peosbar, 
                    RealVector& delta_chi_w0, const bool is_predictor) 
{
    ...
    // do r=0,nr_fine-1
        p0_nph[r] = 0.5*(p0_old[r] + p0_new[r]);
        rho0_nph[r] = 0.5*(rho0_old[r] + rho0_new[r]);
        gamma1bar_nph[r] = 0.5*(gamma1bar_old[r] + gamma1bar_new[r]);
    }
    ...
    for (auto r = 1; r <= nr_fine; ++r) {
        ...
        Real volume_discrepancy = rho0_old[r-1] > base_cutoff_density ? dpdt_factor * p0_minus_peosbar[r-1]/dt : 0.0;
    ...
    for (auto r = 1; r <= max_cutoff+1; ++r) {
        w0[r] = u[r] / (r_edge_loc[r]*r_edge_loc[r]) + w0_from_Sbar[r];
    }

    // do r=max_cutoff+2,nr_fine
    for (auto r = max_cutoff+2; r <= nr_fine; ++r) {
        w0[r] = w0[max_cutoff+1] * r_edge_loc[max_cutoff+1]*r_edge_loc[max_cutoff+1]/(r_edge_loc[r]*r_edge_loc[r]);
    }
    ...
    for (auto r = 0; r < nr_fine; ++r) {
        ...
        w0_new_cen[r] = 0.5 * (w0    [r] + w0    [r+1]);
        Real w0_avg = 0.5 * (dt *  w0_old_cen[r] + dtold *  w0_new_cen[r]) / dt_avg;
        Real div_avg = 0.5 * (dt * (w0_old[r+1]-w0_old[r]) + dtold * (w0[r+1]-w0[r])) / dt_avg;

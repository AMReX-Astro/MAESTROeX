...
void Maestro::ReplaceSbar(BaseState<Real>& Sbar,
                    ...
                    const Real dtold_in)
{
    ...
    const auto gamma1bar_arr = gamma1bar_in.const_array();
    const auto p0_arr = p0_in.const_array();

    const Real dt_loc = dtold_in;
    ...
    // zero Sbar where there is no corresponding full state array
    for (auto n = 1; n <= base_geom.max_radial_level; ++n) {
        for (auto j = 1; j <= base_geom.numdisjointchunks(n); ++j) {
            if (j == base_geom.numdisjointchunks(n)) {
                ...
                for (auto r = base_geom.r_end_coord(n, j) + 1;
    ...
    RestrictBase(Sbar, false);
    FillGhostBase(Sbar, false);
}

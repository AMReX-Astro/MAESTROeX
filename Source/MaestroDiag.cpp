...
#pragma omp parallel reduction(+:kin_ener_level) reduction(+:int_ener_level) reduction(+:nuc_ener_level) reduction(max:U_max_level) reduction(max:Mach_max_level)
    ...
    if (spherical) {
        ...
        m[0] = 4.0/3.0 * M_PI * rho0[0] * r_cc_loc_p(0,0)*r_cc_loc_p(0,0)*r_cc_loc_p(0,0);

        // dU = - G M dM / r;  dM = 4 pi r**2 rho dr  -->  dU = - 4 pi G r rho dr
        grav_ener = -4.0 * M_PI * Gconst * m[0] * r_cc_loc_p(0,0) * rho0[0] * (r_edge_loc_p(0,1) - r_edge_loc_p(0,0));
        ...
        for (auto r = 1; r < nr_fine; ++r) {
            ...
            grav_ener -= 4.0*M_PI*Gconst*m[r]*r_cc_loc_p(0,r) * rho0[max_lev*r]*(r_edge_loc_p(0,r+1)-r_edge_loc_p(0,r));
        }
#endif
    } else {
        // diag_grav_energy(&grav_ener, rho0_in.dataPtr(), r_cc_loc.dataPtr(), r_edge_loc.dataPtr());
        for (auto r = 0; r < nr_fine; ++r) {
            Real dr_loc = r_edge_loc_b.array()(0,r+1) - r_edge_loc_b.array()(0,r);
            grav_ener -= rho0_in[max_lev*r] * r_cc_loc_b.array()(0,r) * grav_const * dr_loc;
        }
    }

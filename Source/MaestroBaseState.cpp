                       ...
                       const int lev)
{
    ...
    for (auto r = 0; r < base_geom.nr(n); ++r) {
        ...
        if (rloc > base_cutoff_density_loc) {
            ...
            Real d_ambient = input_model.Interpolate(rloc, ModelParser::idens_model);
            Real t_ambient = input_model.Interpolate(rloc, ModelParser::itemp_model);
            Real p_ambient = input_model.Interpolate(rloc, ModelParser::ipres_model);
            ...
            for (auto comp = 0; comp < NumSpec; ++comp) {
                xn_ambient[comp] = max(0.0, min(1.0, 
                    input_model.Interpolate(rloc, ModelParser::ispec_model+comp)));

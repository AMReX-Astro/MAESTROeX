...
void 
Maestro::MakePsiPlanar(RealVector& psi_in) 
{
    // timer for profiling
    ...
    std::fill(psi_in.begin(), psi_in.end(), 0.0);
        ...
        for (auto i = 1; i <= numdisjointchunks[n]; ++i){
            for (auto r = r_start_coord[n+max_lev*i]; 
                ...
                if (r < base_cutoff_density_coord[n]) {
                    psi_in[n+max_lev*r] = etarho_cc[n+max_lev*r] * fabs(grav_const);
                }
            }
        }
    }

    RestrictBase(psi_in, true);
    FillGhostBase(psi_in, true);
}

void 
Maestro::MakePsiSphr(RealVector& psi_in, const RealVector& w0_in, 
                     ...
                     const RealVector& Sbar_in) 
{
    ...
    Real * AMREX_RESTRICT psi_p = psi_in.dataPtr();
...
void 
Maestro::MakePsiIrreg(RealVector& psi_in, const RealVector& grav_cell) 
{
    // timer for profiling
    ...
    for (auto r = base_cutoff_density_coord[0]+1; r < nr_fine; ++r) {
        psi_in[max_lev*r] = psi_in[max_lev*(r-1)];
    }

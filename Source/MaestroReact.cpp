...
#ifndef SDC void Maestro::Burner(
    const Vector <MultiFab>& s_in, Vector <MultiFab>& s_out,
    const Vector <MultiFab>& rho_Hext, Vector <MultiFab>& rho_omegadot,
    Vector <MultiFab>& rho_Hnuc, const BaseState <Real>& p0, const Real dt_in,
    const Real time_in) {
        // timer for profiling
        BL_PROFILE_VAR("Maestro::Burner()", Burner);
...
#pragma gpu box(tileBox)
                    burner_loop_sphr(
                        ...
                        time_in, BL_TO_FORTRAN_ANYD(mask[mfi]), (int)use_mask);
                } else {
#pragma gpu box(tileBox)
                    burner_loop(AMREX_INT_ANYD(tileBox.loVect()),
                                AMREX_INT_ANYD(tileBox.hiVect()), lev,
...
#endif
>>>>>>> 8c56219957a6fc5b282f9e387ef4c1534a035c30
    ...
    void Maestro::MakeReactionRates(Vector<MultiFab>& rho_omegadot,
        ...
        const auto ispec_threshold =
            network_spec_index(burner_threshold_species);
                ...
                for (MFIter mfi(scal[lev], TilingIfNotGPU()); mfi.isValid();
                    ...
                    ParallelFor(tilebox, [=] AMREX_GPU_DEVICE(int i, int j,
                        ...
                        if ((rho > burning_cutoff_density_lo &&
                            ...
                            for (auto n = 0; n < NumSpec; ++n) {
                                eos_state.xn[n] =
                                    scal_arr(i, j, k, FirstSpec + n) /
                                    eos_state.rho;
                            }
                            eos_state.h =
                                scal_arr(i, j, k, RhoH) / eos_state.rho;

        ...
        if (i .eq. lo(1) .and. lo(1) .eq. domlo(1)) then
           if (adv_bc(1,1,bccomp) .eq. EXT_DIR  .or. adv_bc(1,1,bccomp) .eq. HOEXTRAP) then
            ...
            ! make sure sedge lies in between adjacent cell-centered values
              ...
              ! use a modified stencil to get sedge on the first interior edge
               sedge = &
                   -FIFTH        *s(lo(1)-1,j,k,n) &
                   + (THREE/FOUR)*s(lo(1),j,k,n) &
                   + HALF        *s(lo(1)+1,j,k,n) &
                   - (ONE/20.0d0)*s(lo(1)+2,j,k,n)

               sedge = max(sedge,min(s(lo(1)+1,j,k,n),s(lo(1),j,k,n)))
               sedge = min(sedge,max(s(lo(1)+1,j,k,n),s(lo(1),j,k,n)))
        ...
        if (i .eq. hi(1) .and. hi(1) .eq. domhi(1)) then
           if (adv_bc(1,2,bccomp) .eq. EXT_DIR  .or. adv_bc(1,2,bccomp) .eq. HOEXTRAP) then
            ...
            ! make sure sedge lies in between adjacent cell-centered values
              ! use a modified stencil to get sedge on the first interior edge
              sedge = &
                   -FIFTH        *s(hi(1)+1,j,k,n) &
                   + (THREE/FOUR)*s(hi(1),j,k,n) &
                   + HALF        *s(hi(1)-1,j,k,n) &
                   - (ONE/20.0d0)*s(hi(1)-2,j,k,n)

               sedge = max(sedge,min(s(hi(1)-1,j,k,n),s(hi(1),j,k,n)))
               sedge = min(sedge,max(s(hi(1)-1,j,k,n),s(hi(1),j,k,n)))
            ...
            if (adv_bc(1,1,bccomp) .eq. EXT_DIR  .or. adv_bc(1,1,bccomp) .eq. HOEXTRAP) then
              ...
              if (i .eq. lo(1)) then
                ...
                sedger = &
                 ...
                 sedger = max(sedger,min(s(lo(1)+1,j,k,n),s(lo(1),j,k,n)))
                 sedger = min(sedger,max(s(lo(1)+1,j,k,n),s(lo(1),j,k,n)))
              ...
              elseif (i .eq. lo(1)+2) then
                ...
                sedgel = &
                 ...
                 sedgel = max(sedgel,min(s(lo(1)+1,j,k,n),s(lo(1),j,k,n)))
                 sedgel = min(sedgel,max(s(lo(1)+1,j,k,n),s(lo(1),j,k,n)))
            ...
            if (adv_bc(1,2,bccomp) .eq. EXT_DIR  .or. adv_bc(1,2,bccomp) .eq. HOEXTRAP) then
              ...
              elseif (i .eq. hi(1)-1) then
                ...
                sedger = &
                 ...
                 sedger = max(sedger,min(s(hi(1)-1,j,k,n),s(hi(1),j,k,n)))
                 sedger = min(sedger,max(s(hi(1)-1,j,k,n),s(hi(1),j,k,n)))
              ...
              elseif (i .eq. hi(1)-2) then
                ...
                sedgerr = &
                 ...
                 sedgerr = max(sedgerr,min(s(hi(1)-1,j,k,n),s(hi(1),j,k,n)))
                 sedgerr = min(sedgerr,max(s(hi(1)-1,j,k,n),s(hi(1),j,k,n)))
    ...
    if (ppm_type .eq. 1) then
       ...
       if (j .eq. lo(2) .and. lo(2) .eq. domlo(2)) then
          if (adv_bc(2,1,bccomp) .eq. EXT_DIR  .or. adv_bc(2,1,bccomp) .eq. HOEXTRAP) then
             ...
             ! do i=lo(1)-1,hi(1)+1
               ...
               ! use a modified stencil to get sedge on the first interior edge
               sedge = &
                    -FIFTH        *s(i,lo(2)-1,k,n) &
                    + (THREE/FOUR)*s(i,lo(2),k,n) &
                    + HALF        *s(i,lo(2)+1,k,n) &
                    - (ONE/20.0d0)*s(i,lo(2)+2,k,n)

                sedge = max(sedge,min(s(i,lo(2)+1,k,n),s(i,lo(2),k,n)))
                sedge = min(sedge,max(s(i,lo(2)+1,k,n),s(i,lo(2),k,n)))
             !
             ! ! copy sedge into sp and sm
                sp = sedge
          end if
       ...
       if (j .eq. hi(2) .and. hi(2) .eq. domhi(2)) then
          if (adv_bc(2,2,bccomp) .eq. EXT_DIR  .or. adv_bc(2,2,bccomp) .eq. HOEXTRAP) then
                ...
                ! use a modified stencil to get sedge on the first interior edge
                sedge = &
                     -FIFTH        *s(i,hi(2)+1,k,n) &
                     + (THREE/FOUR)*s(i,hi(2),k,n) &
                     + HALF        *s(i,hi(2)-1,k,n) &
                     - (ONE/20.0d0)*s(i,hi(2)-2,k,n)

                sedge = max(sedge,min(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))
                sedge = min(sedge,max(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))
             !
             ! ! copy sedge into sp and sm
                sm = sedge
          ...
          if (adv_bc(2,1,bccomp) .eq. EXT_DIR  .or. adv_bc(2,1,bccomp) .eq. HOEXTRAP) then
             ...
             if (j .eq. lo(2)) then
               ...
               sp = &
                ...
                sp = max(sp,min(s(i,lo(2)+1,k,n),s(i,lo(2),k,n)))
                sp = min(sp,max(s(i,lo(2)+1,k,n),s(i,lo(2),k,n)))

             elseif (j .eq. lo(2)+1) then

               sedgel = s(i,lo(2)-1,k,n)
          ...
          if (adv_bc(2,2,bccomp) .eq. EXT_DIR  .or. adv_bc(2,2,bccomp) .eq. HOEXTRAP) then
            ...
            if (j .eq. hi(2)) then
              ...
              sm = &
               ...
               sm = max(sm,min(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))
               sm = min(sm,max(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))

            elseif (j .eq. hi(2)-1) then

              sedgerr = s(i,hi(2)+1,k,n)
             ...
             sedger = max(sedger,min(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))
             sedger = min(sedger,max(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))
            ...
            elseif (j .eq. hi(2)-2) then
             ...
             sedgerr = max(sedgerr,min(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))
             sedgerr = min(sedgerr,max(s(i,hi(2)-1,k,n),s(i,hi(2),k,n)))

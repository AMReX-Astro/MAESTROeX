     ...
     do k=lo(3)-1,hi(3)+1
        do j=lo(2)-1,hi(2)+1
           do i=lo(1)-1,hi(1)+1
              ...
              ! Interpolate s to x-edges.
              !
              sm(i,j,k) = HALF*(s(i,j,k)+s(i-1,j,k)) - SIXTH*(dsvl_r-dsvl_l)
              !
              ! Make sure sedge lies in between adjacent cell-centered values.
              !
              sm(i,j,k) = max(sm(i,j,k),min(s(i,j,k),s(i-1,j,k)))
              sm(i,j,k) = min(sm(i,j,k),max(s(i,j,k),s(i-1,j,k)))
              ...
              ! Interpolate s to x-edges.
              !
              sp(i,j,k) = HALF*(s(i+1,j,k)+s(i,j,k)) - SIXTH*(dsvl_r-dsvl_l)
              !
              ! Make sure sedge lies in between adjacent cell-centered values.
              !
              sp(i,j,k) = max(sp(i,j,k),min(s(i+1,j,k),s(i,j,k)))
              sp(i,j,k) = min(sp(i,j,k),max(s(i+1,j,k),s(i,j,k)))
     !       end do
     !    end do
     ! end do
     ! !$OMP END DO
     ...
     do k=lo(3)-1,hi(3)+1
        do j=lo(2)-1,hi(2)+1
           do i=lo(1)-1,hi(1)+1
              ...
              ! Interpolate s to y-edges.
              !
              sm(i,j,k) = HALF*(s(i,j,k)+s(i,j-1,k)) - SIXTH*(dsvl_r-dsvl_l)
              !
              ! Make sure sedge lies in between adjacent cell-centered values.
              !
              sm(i,j,k) = max(sm(i,j,k),min(s(i,j,k),s(i,j-1,k)))
              sm(i,j,k) = min(sm(i,j,k),max(s(i,j,k),s(i,j-1,k)))
              ...
              ! Interpolate s to y-edges.
              !
              sp(i,j,k) = HALF*(s(i,j+1,k)+s(i,j,k)) - SIXTH*(dsvl_r-dsvl_l)
              !
              ! Make sure sedge lies in between adjacent cell-centered values.
              !
              sp(i,j,k) = max(sp(i,j,k),min(s(i,j+1,k),s(i,j,k)))
              sp(i,j,k) = min(sp(i,j,k),max(s(i,j+1,k),s(i,j,k)))
     ...
     do k=lo(3)-1,hi(3)+1
        do j=lo(2)-1,hi(2)+1
           do i=lo(1)-1,hi(1)+1
              ...
              !
              sm(i,j,k) = HALF*(s(i,j,k)+s(i,j,k-1)) - SIXTH*(dsvl_r-dsvl_l)
              !
              ! Make sure sedge lies in between adjacent cell-centered values.
              !
              sm(i,j,k) = max(sm(i,j,k),min(s(i,j,k),s(i,j,k-1)))
              sm(i,j,k) = min(sm(i,j,k),max(s(i,j,k),s(i,j,k-1)))
              ...
              ! Interpolate s to z-edges.
              !
              sp(i,j,k) = HALF*(s(i,j,k+1)+s(i,j,k)) - SIXTH*(dsvl_r-dsvl_l)
              !
              ! Make sure sedge lies in between adjacent cell-centered values.
              !
              sp(i,j,k) = max(sp(i,j,k),min(s(i,j,k+1),s(i,j,k)))
              sp(i,j,k) = min(sp(i,j,k),max(s(i,j,k+1),s(i,j,k)))

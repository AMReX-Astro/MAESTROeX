module enforce_HSE_module
  ...
  use meth_params_module, only: do_planar_invsq_grav, do_2d_planar_octant
...
contains
  ...
  subroutine enforce_HSE(rho0,p0,grav_cell,r_edge_loc) bind(C, name="enforce_HSE")
    ...
    do r=1,min(r_end_coord(0,1),base_cutoff_density_coord(0))
       p0(0,r) = p0(0,r-1) + (dr(0)/2.d0)*(rho0(0,r)+rho0(0,r-1))*grav_edge(0,r)
    end do

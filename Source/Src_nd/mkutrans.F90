...
!        if (ppm_type .eq. 0) then
!           s_hi(1) = s_hi(1)-1
!           call bl_allocate(slopex,s_lo,s_hi,1)
!            do k = lo(3)-1,hi(3)+1
!               call slopex_2d(utilde(:,:,k,1:1),slopex(:,:,k,:),domlo,domhi,lo,(/hi(1)-1,hi(2),hi(3)/),ng_ut,1,adv_bc(:,:,1:1))
!            end do
!
!           ! call slopex_2d(s_lo,s_hi,utilde(:,:,:,1:1),ut_lo,ut_hi,1, &
!           !       slopex,s_lo,s_hi,1, &
!           !      domlo,domhi,1,adv_bc,AMREX_SPACEDIM,1)
!        end if
!        !******************************************************************
!        ! create utrans
!        !******************************************************************
...
!        if (ppm_type .eq. 0) then
!           s_hi(2) = s_hi(2)-1
!           call bl_allocate(slopey,s_lo,s_hi,1)
!
!           do k = lo(3)-1,hi(3)+1
!               call slopey_2d(utilde(:,:,k,2:2),slopey(:,:,k,:),domlo,domhi,lo,(/hi(1),hi(2)-1,hi(3)/),ng_ut,1,adv_bc(:,:,2:2))
!            end do
!
!           ! call slopey_2d(s_lo,s_hi,utilde(:,:,:,2:2),ut_lo,ut_hi,1, &
!           !       slopey,s_lo,s_hi,1, &
...
!        if (ppm_type .eq. 0) then
!           s_hi(3) = s_hi(3)-1
!           call bl_allocate(slopez,s_lo,s_hi,1)
!
!           ! call slopez_3d(utilde(:,:,:,3:3),1,slopez,1, &
!           !      domlo,domhi,lo,(/hi(1),hi(2),hi(3)-1/),ng_ut,1,adv_bc(:,:,3:3),1)
!           call slopez_3d(utilde(:,:,:,3:3),slopez,domlo,domhi,lo,(/hi(1),hi(2),hi(3)-1/),ng_ut,1,adv_bc(:,:,3:3))
!        end if
!        !******************************************************************
!        ! create wtrans
!        !******************************************************************

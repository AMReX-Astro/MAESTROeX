  ...
  contains
    ...
    subroutine mkutrans_2d(lev, domlo, domhi, lo, hi, &
      ...
      allocate(slopex(lo(1)-1:hi(1)+1,lo(2)-1:hi(2)+1,1,1))
      allocate(slopey(lo(1)-1:hi(1)+1,lo(2)-1:hi(2)+1,1,1))
      ...
      do j=lo(2),hi(2)
         do i=lo(1),hi(1)+1
            ...
            if (i .eq. lo(1) .and. lo(1) .eq. domlo(1)) then
               select case(phys_bc(1,1))
               case (Inflow)
                  ulx(lo(1),j) = utilde(lo(1)-1,j,1)
                  urx(lo(1),j) = utilde(lo(1)-1,j,1)
               case (SlipWall, NoSlipWall, Symmetry)
                  ulx(lo(1),j) = ZERO
                  urx(lo(1),j) = ZERO
               case (Outflow)
                  ulx(lo(1),j) = min(urx(lo(1),j),ZERO)
                  urx(lo(1),j) = ulx(lo(1),j)
               case (Interior)
               case  default
#ifndef AMREX_GPU
            ...
            if (i .eq. hi(1) .and. hi(1) .eq. domhi(1)) then
               select case(phys_bc(1,2))
               case (Inflow)
                  ulx(hi(1)+1,j) = utilde(hi(1)+1,j,1)
                  urx(hi(1)+1,j) = utilde(hi(1)+1,j,1)
               case (SlipWall, NoSlipWall, Symmetry)
                  ulx(hi(1)+1,j) = ZERO
                  urx(hi(1)+1,j) = ZERO
               case (Outflow)
                  ulx(hi(1)+1,lj) = max(ulx(hi(1)+1,j),ZERO)
                  urx(hi(1)+1,j) = ulx(hi(1)+1,j)
               case (Interior)
               case  default
#ifndef AMREX_GPU
      ...
      if (lo(2) .eq. domlo(2)) then
         ...
         case (Inflow)
            vly(lo(1):hi(1),lo(2)) = utilde(lo(1):hi(1),lo(2)-1,2)
            vry(lo(1):hi(1),lo(2)) = utilde(lo(1):hi(1),lo(2)-1,2)
         case (SlipWall, NoSlipWall, Symmetry)
            vly(lo(1):hi(1),lo(2)) = ZERO
            vry(lo(1):hi(1),lo(2)) = ZERO
         case (Outflow)
            vly(lo(1):hi(1),lo(2)) = min(vry(lo(1):hi(1),lo(2)),ZERO)
            vry(lo(1):hi(1),lo(2)) = vly(lo(1):hi(1),lo(2))
         case (Interior)
         case  default
#ifndef AMREX_GPU
      ...
      if (hi(2) .eq. domhi(2)) then
         ...
         case (Inflow)
            vly(lo(1):hi(1),hi(2)+1) = utilde(lo(1):hi(1),hi(2)+1,2)
            vry(lo(1):hi(1),hi(2)+1) = utilde(lo(1):hi(1),hi(2)+1,2)
         case (SlipWall, NoSlipWall, Symmetry)
            vly(lo(1):hi(1),hi(2)+1) = ZERO
            vry(lo(1):hi(1),hi(2)+1) = ZERO
         case (Outflow)
            vly(lo(1):hi(1),hi(2)+1) = max(vly(lo(1):hi(1),hi(2)+1),ZERO)
            vry(lo(1):hi(1),hi(2)+1) = vly(lo(1):hi(1),hi(2)+1)
         case (Interior)
         case  default
#ifndef AMREX_GPU

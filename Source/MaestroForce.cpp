...
void
Maestro::MakeVelForce (Vector<MultiFab>& vel_force_in,
                       ...
                       int do_add_utilde_force)
{
    ...
    // Reset vel_force
    for (int lev=0; lev<=finest_level; ++lev) {
	vel_force_in[lev].setVal(0.);
    }

    for (int lev=0; lev<=finest_level; ++lev) {

        // get references to the MultiFabs at level lev
        MultiFab& vel_force_mf = vel_force_in[lev];
...
#pragma omp parallel
    ...
    AverageDown(vel_force_in,0,AMREX_SPACEDIM);

    // note - we need to reconsider the bcs type here
    // it matches fortran MAESTRO but is that correct?
    FillPatch(t_old, vel_force_in, vel_force_in, vel_force_in, 0, 0, AMREX_SPACEDIM, 0,
              bcs_u, 1);

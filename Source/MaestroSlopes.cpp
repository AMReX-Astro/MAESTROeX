...
void Maestro::Slopex(const Box& bx, Array4<Real> const s,
    ...
    if (slope_order == 0) {
        ...
        AMREX_PARALLEL_FOR_4D(bx, ncomp, i, j, k, n, {
            ...
            Real dlim = min(amrex::Math::abs(dmin), amrex::Math::abs(dpls));
            dlim = dpls * dmin > 0.0 ? dlim : 0.0;
            Real dflag = amrex::Math::copysign(1.0, dcen);
            Real dxl = dflag * min(dlim, amrex::Math::abs(dcen));

            // right
            dcen = 0.5 * (s(i + 2, j, k, n) - s(i, j, k, n));
            dmin = 2.0 * (s(i + 1, j, k, n) - s(i, j, k, n));
            dpls = 2.0 * (s(i + 2, j, k, n) - s(i + 1, j, k, n));
            dlim = min(amrex::Math::abs(dmin), amrex::Math::abs(dpls));
            dlim = dpls * dmin > 0.0 ? dlim : 0.0;
            dflag = amrex::Math::copysign(1.0, dcen);
            Real dxr = dflag * min(dlim, amrex::Math::abs(dcen));
            ...
            slx(i, j, k, n) = dflag * min(amrex::Math::abs(ds), dlim);
            ...
            if (bclo_p[n] == EXT_DIR || bclo_p[n] == HOEXTRAP) {
                if (i == ilo - 1) {
                    ...
                    Real del =
                        -16.0 / 15.0 * s(i - 1, j, k, n) + 0.5 * s(i, j, k, n) +
            ...
            if (bchi_p[n] == EXT_DIR || bchi_p[n] == HOEXTRAP) {
                if (i == ihi + 1) {
                    ...
                    Real del =
                        -(-16.0 / 15.0 * s(i + 1, j, k, n) +
                          0.5 * s(i, j, k, n) + 2.0 / 3.0 * s(i - 1, j, k, n) -
                          0.1 * s(i - 2, j, k, n));
                    dmin = 2.0 * (s(i, j, k, n) - s(i - 1, j, k, n));
                    dpls = 2.0 * (s(i + 1, j, k, n) - s(i, j, k, n));
                    Real slim =
                    ...
                    Real del =
                        -(-16.0 / 15.0 * s(i + 2, j, k, n) +
                          0.5 * s(i + 1, j, k, n) + 2.0 / 3.0 * s(i, j, k, n) -
                          0.1 * s(i - 1, j, k, n));
                    dmin = 2.0 * (s(i + 1, j, k, n) - s(i, j, k, n));
                    dpls = 2.0 * (s(i + 2, j, k, n) - s(i + 1, j, k, n));
                    Real slim =
...
void Maestro::Slopey(const Box& bx, Array4<Real> const s,
    ...
    if (slope_order == 0) {
        ...
        AMREX_PARALLEL_FOR_4D(bx, ncomp, i, j, k, n, {
            ...
            dlim = dpls * dmin > 0.0 ? dlim : 0.0;
            Real dflag = amrex::Math::copysign(1.0, dcen);
            Real dyl = dflag * min(dlim, amrex::Math::abs(dcen));
            ...
            dlim = dpls * dmin > 0.0 ? dlim : 0.0;
            dflag = amrex::Math::copysign(1.0, dcen);
            Real dyr = dflag * min(dlim, amrex::Math::abs(dcen));
            ...
            sly(i, j, k, n) = dflag * min(amrex::Math::abs(ds), dlim);
            ...
            if (bclo_p[n] == EXT_DIR || bclo_p[n] == HOEXTRAP) {
                if (j == jlo - 1) {
                    ...
                    Real del =
                        -16.0 / 15.0 * s(i, j - 1, k, n) + 0.5 * s(i, j, k, n) +
            ...
            if (bchi_p[n] == EXT_DIR || bchi_p[n] == HOEXTRAP) {
                if (j == jhi + 1) {
                    ...
                    Real del =
                        -(-16.0 / 15.0 * s(i, j + 1, k, n) +
                          0.5 * s(i, j, k, n) + 2.0 / 3.0 * s(i, j - 1, k, n) -
                          0.1 * s(i, j - 2, k, n));
                    dmin = 2.0 * (s(i, j, k, n) - s(i, j - 1, k, n));
                    dpls = 2.0 * (s(i, j + 1, k, n) - s(i, j, k, n));
                    Real slim =
                    ...
                    Real del =
                        -(-16.0 / 15.0 * s(i, j + 2, k, n) +
                          0.5 * s(i, j + 1, k, n) + 2.0 / 3.0 * s(i, j, k, n) -
                          0.1 * s(i, j - 1, k, n));
                    dmin = 2.0 * (s(i, j + 1, k, n) - s(i, j, k, n));
                    dpls = 2.0 * (s(i, j + 2, k, n) - s(i, j + 1, k, n));
                    Real slim =
...
void Maestro::Slopez(const Box& bx, Array4<Real> const s,
    ...
    if (slope_order == 0) {
        ...
        AMREX_PARALLEL_FOR_4D(bx, ncomp, i, j, k, n, {
            ...
            dlim = dpls * dmin > 0.0 ? dlim : 0.0;
            Real dflag = amrex::Math::copysign(1.0, dcen);
            Real dzl = dflag * min(dlim, amrex::Math::abs(dcen));
            ...
            dlim = dpls * dmin > 0.0 ? dlim : 0.0;
            dflag = amrex::Math::copysign(1.0, dcen);
            Real dzr = dflag * min(dlim, amrex::Math::abs(dcen));
            ...
            slz(i, j, k, n) = dflag * min(amrex::Math::abs(ds), dlim);
            ...
            if (bclo_p[n] == EXT_DIR || bclo_p[n] == HOEXTRAP) {
                if (k == klo - 1) {
                    ...
                    Real del =
                        -16.0 / 15.0 * s(i, j, k - 1, n) + 0.5 * s(i, j, k, n) +
            ...
            if (bchi_p[n] == EXT_DIR || bchi_p[n] == HOEXTRAP) {
                if (k == khi + 1) {
                    ...
                    Real del =
                        -(-16.0 / 15.0 * s(i, j, k + 1, n) +
                          0.5 * s(i, j, k, n) + 2.0 / 3.0 * s(i, j, k - 1, n) -
                          0.1 * s(i, j, k - 2, n));
                    dmin = 2.0 * (s(i, j, k, n) - s(i, j, k - 1, n));
                    dpls = 2.0 * (s(i, j, k + 1, n) - s(i, j, k, n));
                    Real slim =

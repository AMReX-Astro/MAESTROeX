                    ...
                    const RealVector& grav_cell) 
{
            ...
            for (auto i = 1; i <= numdisjointchunks_b(n); ++i) {
                ...
                if (r_start_coord_b(n,i) == 0) {
                    ...
                    if (do_planar_invsq_grav || do_2d_planar_octant) {
                        ...
                        p0[n+max_lev*r_start_coord_b(n,i)] = p0[n-1+max_lev*(r_start_coord_b(n,i)/2-1)]
                                + (dr[n]/4.0)* 
                                (2.0*rho0[n+max_lev*r_start_coord_b(n,i)]/3.0 + 
                                4.0*rho0[n-1+max_lev*(r_start_coord_b(n,i)/2-1)]/3.0)* 
                                (grav_edge[n+max_lev*r_start_coord_b(n,i)] + 
                                grav_cell[n-1+max_lev*(r_start_coord_b(n,i)/2-1)])  
                                + (dr[n]/8.0)* 
                                (5.0*rho0[n+max_lev*r_start_coord_b(n,i)]/3.0 + 
                                1.0*rho0[n-1+max_lev*(r_start_coord_b(n,i)/2-1)]/3.0)* 
                                (grav_edge[n+max_lev*r_start_coord_b(n,i)] + 
                                grav_cell[n+max_lev*r_start_coord_b(n,i)]);
                    } else {
                        // assuming constant g here
                        p0[n+max_lev*r_start_coord_b(n,i)] = p0[n-1+max_lev*(r_start_coord_b(n,i)/2-1)] 
                                + (3.0*grav_cell[1]*dr[n]/4.0)* 
                                (rho0[n-1+max_lev*(r_start_coord_b(n,i)/2-1)]+rho0[n+max_lev*r_start_coord_b(n,i)]);
                    }
                } else {
                ...
                if (r_end_coord_b(n,i) == nr[n]-1) {
                    ...
                    if (do_planar_invsq_grav || do_2d_planar_octant) {
                        // we have variable gravity
                        Real temp = p0[n+max_lev*r_end_coord_b(n,i)] 
                                + (dr[n]/4.0)* 
                                (2.0*rho0[n+max_lev*r_end_coord_b(n,i)]/3.0 + 
                                4.0*rho0[n-1+max_lev*(r_end_coord_b(n,i)+1)/2]/3.0)* 
                                (grav_edge[n-1+max_lev*(r_end_coord_b(n,i)+1)/2] + 
                                grav_cell[n-1+max_lev*(r_end_coord_b(n,i)+1)/2]) 
                                + (dr[n]/8.0)* 
                                (5.0*rho0[n+max_lev*r_end_coord_b(n,i)]/3.0 + 
                                1.0*rho0[n-1+max_lev*(r_end_coord_b(n,i)+1)/2]/3.0)* 
                                (grav_cell[n+max_lev*r_end_coord_b(n,i)] + 
                                grav_edge[n-1+max_lev*(r_end_coord_b(n,i)+1)/2]);
                        offset = p0[n-1+max_lev*(r_end_coord_b(n,i)+1)/2] - temp;
                    } else {
                        // assuming constant g here
                        Real temp = p0[n+max_lev*r_end_coord_b(n,i)] + (3.0*grav_cell[1]*dr[n]/4.0)* 
                                (rho0[n+max_lev*r_end_coord_b(n,i)]+rho0[n-1+max_lev*(r_end_coord_b(n,i)+1)/2]);
                        offset = p0[n-1+max_lev*(r_end_coord_b(n,i)+1)/2] - temp;
                    }
                } else {
                    // copy pressure from below
                ...
                if (r_end_coord_b(n,i) != nr[n]-1) {
                    for (auto l = n-1; l >= 0; --l) {
                        for (int r = (r_end_coord_b(n,i)+1)/pow(2, n-l); 
                             r <= nr[l]-1; ++r) {
    ...
    for (auto n = 1; n <= finest_radial_level; ++n) {
        for (auto i = 1; i <= numdisjointchunks_b(n); ++i) {
            for (auto r = r_start_coord_b(n,i); r <= r_end_coord_b(n,i); ++r) {
    ...
    for (auto n = 1; n <= finest_radial_level; ++n) {
        for (auto i = 1; i <= numdisjointchunks_b(n); ++i) {
            if (i == numdisjointchunks_b(n)) {
                for (auto r = r_end_coord_b(n,i)+1; r < nr[n]; ++r) {
                    p0[n+max_lev*r] = 0.0;
                }
            } else {
                for (auto r = r_end_coord_b(n,i)+1; r < r_start_coord_b(n,i+1); ++r) {
                    p0[n+max_lev*r] = 0.0;
                }
            }
        }

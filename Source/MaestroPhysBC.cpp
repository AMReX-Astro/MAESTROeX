...
MaestroPhysBC::MaestroPhysBC (const Geometry& geom, const Array<BCRec>& bcr)
    : m_geom(geom), m_bcr(bcr)
{ }

void
MaestroPhysBC::define (const Geometry& geom, const Array<BCRec>& bcr)
{
    m_geom = geom;
    m_bcr = bcr;
}
...
void
MaestroPhysBC::FillBoundary (MultiFab& mf, int dcomp, int ncomp, Real time)
{
    BL_PROFILE("MaestroPhysBC::FillBoundary");
